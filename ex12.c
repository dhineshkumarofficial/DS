...
void main()
{
    ...
    scanf("%d", &n);
    /* Read the names and roll numbers */
    for (i = 0; i < n; i++) {
        printf("\nEnter name and roll number for student %d : ", i);
        scanf("%s", &st[i].name);
        scanf("%d", &st[i].rollno);
    }
    /* Read ith student's jth subject marks */
    for (i = 0; i < n; i++) {
        for (j = 0; j <= 2; j++) {
    ...
    /* List roll numbers who secured the highest marks in each subject */
    for (j = 0; j < 3; j++) {
        max = 0;
        for (i = 0; i < n; i++) {
            if (max < st[i].sub[j]) {
        ...
        printf("\nStudent %s got maximum marks = %d in Subject : %d",
               st[ni].name, max, j);
    }
